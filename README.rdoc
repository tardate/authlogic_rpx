...
* account_merge_enabled: enable/disable user auto-registration (disabled by default)
* rpx_extended_info: enable/disable extended profile information in the RPX authentication (disabled by default)

For example, the following shows how to set standard Authlogic configurations (validations_scope), and enables RPX account merging:
